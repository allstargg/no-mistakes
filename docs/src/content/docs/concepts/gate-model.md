...
The database also contains an append-only metadata event log with a global monotonic sequence, unique event ID, source and local acceptance timestamps, versioned event and metadata-schema identifiers, optional run linkage, and a validated copy of that run's trace context. Its typed API exposes fixed run, step, invocation, gate, CI, and PR lifecycle metadata with no arbitrary JSON, content, logs, prompts, diffs, provider errors, check names, or URLs. Covered state transitions and their describing events commit in one SQLite transaction, and transition guards suppress duplicate facts across retries and restarts. The daemon offers feature-detected global replay with cursors and bounded filters while retaining the existing per-run subscription as the compatibility path. [Metadata Events](/no-mistakes/reference/metadata-events/) owns event names, fixed fields, bounded outcomes, transaction behavior, and replay semantics. [`event_log_retention`](/no-mistakes/reference/global-config/#event_log_retention) owns bounded startup cleanup.
