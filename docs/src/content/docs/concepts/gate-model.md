...
The database also contains an append-only metadata event log with a global monotonic sequence, unique event ID, source and local acceptance timestamps, versioned event and payload-schema identifiers, optional run linkage, and a validated copy of that run's trace context. Its typed append API is metadata-only and exposes no arbitrary JSON or content channel. The field bounds and `io.no_mistakes.*.vN` identifiers align with the `tracewake.events.v1` source contract. A minimal in-scope set of run, step, gate, and invocation mutations commits its state change and the metadata event describing it in one SQLite transaction, so committed state never lacks its event and no event claims state that did not commit; broader lifecycle coverage and global replay subscriptions are not yet part of the interface, so external consumers continue using the existing per-run subscription as their degradation path. [`event_log_retention`](/no-mistakes/reference/global-config/#event_log_retention) owns its bounded startup cleanup policy.
