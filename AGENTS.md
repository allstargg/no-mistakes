...
- `DB.CommitWithEvent` is the single transaction abstraction that commits a state mutation and the TW-33 metadata event describing it in one SQLite transaction; its doc comment owns the scope/ordering/error/rollback/retry semantics. The mutate closure MUST write only through the passed `*sql.Tx` (the pool is `SetMaxOpenConns(1)`, so a stray `d.sql` call inside it deadlocks). Event input is validated before BEGIN; the mutation runs before the append so run/trace linkage reads post-mutation state.
- The in-scope coupled set is intentionally minimal - one authoritative single-caller mutation per state family: `InsertRunWithEvent` (run.created), `StartStepWithEvent` (step.started), `SetRunAwaitingAgentWithEvent` (gate.awaiting_agent), `InsertAgentInvocationWithEvent` (invocation.recorded). Broader lifecycle coverage is TW-34; do not invent events to widen coverage. Plain methods stay compatible and share the exact SQL via extracted `*Exec` helpers (`insertRunExec`, `startStepExec`, `setRunAwaitingAgentExec`, `insertAgentInvocationExec`), so there is no parallel state-write owner. The invocation write detaches from caller cancellation (`context.WithoutCancel`) so a cancelled invocation is still recorded.
- Regressions: `internal/db/events_tx_test.go` (counterfactual, primitive fault/rollback, per-family atomicity, concurrency, restart consistency), e2e `TestTraceContextJourney`.

**Global Replayable Event Subscription (TW-37)**

- The daemon exposes a typed, versioned, feature-detected global subscription over the durable TW-33 log: `MethodCapabilities` (`capabilities`) advertises `CapabilitySubscribeEvents`/`SubscribeEventsVersion` so a consumer selects global mode only when supported and otherwise keeps the unchanged per-run `MethodSubscribe`; an older daemon returns method-not-found, which the client maps to `ErrEventCapabilityUnsupported` (`ipc.IsUnsupportedCapability`). Wire types, the opaque monotonic `nm1:<seq>` cursor, bounded `EventFilter`, `MetadataEventInfo` (metadata-only, no content/payload field — content exclusion by construction), and the typed sentinels live in `internal/ipc/events.go`; `internal/ipc/events_client.go` owns `SubscribeEvents`/`Capabilities`. Setup rejections are `*ipc.CodedError` carried by the server's `errorResponseFor` (honors `RPCCode`), so unknown version / invalid filter / malformed cursor / expired cursor stay distinct typed errors, never generic internal ones.
