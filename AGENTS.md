...
**Transactional State+Event Writes (`internal/db/events_tx.go`)**

- `DB.CommitWithEvent` and its ordered multi-event form `CommitWithEvents` are the transaction abstractions that commit a state mutation and its TW-33 metadata facts in one SQLite transaction; their doc comments own scope/ordering/error/rollback semantics. The mutate closure MUST write only through the passed `*sql.Tx` (the pool is `SetMaxOpenConns(1)`, so a stray `d.sql` call inside it deadlocks). Every event input is validated before BEGIN; the mutation runs before append so run/trace linkage reads post-mutation state.
- Covered owners include run/step creation, invocation completion records, gate enter/exit, CI observations/readiness, and monotonic PR transitions. Transition-guarded methods emit only for a real durable state change, and terminal PR truth atomically commits PR/CI terminal events plus active gate exit and run/CI finalization. Plain legacy methods stay compatible for tests and old recovery data; production lifecycle call sites use the event-coupled owners. The invocation write detaches from caller cancellation (`context.WithoutCancel`) so a cancelled invocation is still recorded.
- Regressions: `internal/db/events_tx_test.go`, `internal/db/lifecycle_event_test.go` (rollback, transition ordering, usage unknowns, allowlists, restart/dedupe, concurrency), e2e `TestTraceContextJourney` / `TestLifecycleEventJourney`.

**Typed Lifecycle Metadata Events (TW-34)**

- `internal/db/event_lifecycle.go` owns event names, fixed metadata shapes, validation, and family-specific storage. Never add a generic payload/attributes JSON channel. Invocation events expose exact reported usage only when known and fixed count-only activity; gate identity persists across restart; CI/PR outcomes use bounded enums and omit check names, URLs, logs, and raw errors. User-facing schema ownership is `docs/src/content/docs/reference/metadata-events.md`.
- Invocation rows commit an ordered started/completed event pair after the attempt ends because no separate in-progress invocation row exists; source timestamps preserve the actual start/end. `invocation_id` correlates the pair. CAS-like state guards, not inferred mutable state, own retry/restart dedupe.

**Global Replayable Event Subscription (TW-37)**

- The daemon exposes a typed, versioned, feature-detected global subscription over the durable TW-33 log: `MethodCapabilities` (`capabilities`) advertises `CapabilitySubscribeEvents`/`SubscribeEventsVersion` so a consumer selects global mode only when supported and otherwise keeps the unchanged per-run `MethodSubscribe`; an older daemon returns method-not-found, which the client maps to `ErrEventCapabilityUnsupported` (`ipc.IsUnsupportedCapability`). Wire types, the opaque monotonic `nm1:<seq>` cursor, bounded `EventFilter`, and `MetadataEventInfo` live in `internal/ipc/events.go`; lifecycle family fields are fixed typed metadata with no generic payload/content/map channel. `internal/ipc/events_client.go` owns `SubscribeEvents`/`Capabilities`. Setup rejections are `*ipc.CodedError` carried by the server's `errorResponseFor` (honors `RPCCode`), so unknown version / invalid filter / malformed cursor / expired cursor stay distinct typed errors, never generic internal ones.
- `internal/daemon/event_subscription.go` owns the daemon side: replay is database-backed (`db.ReadMetadataEvents`) so it survives daemon and client restart, and the live tail is a wake-up-only `eventNotifier` fed by `db.SetEventAppendedHook` (fires post-commit, O(1), independent of any consumer). Cursor expiry is exact and tied to retention: `db.CleanupMetadataEvents` advances the persisted `event_log_state.purged_through` watermark (`db.PurgedThroughSequence`) to the max deleted sequence, and a resume cursor strictly below it fails closed with `ErrCursorExpired`; a fresh (empty-cursor) subscribe replays the retained backlog and never expires.
...
**Native OTLP Trace and Metric Projection (`internal/oteltrace`)**

- Native OTLP is opt-in through standard HTTP/protobuf signal endpoints and reconstructs terminal run, step, gate, CI, failure, and invocation spans plus aggregate invocation metrics only from durable facts. The DB terminal hook is identity-only, O(1), and post-commit; no SDK span object stays live during a pipeline or across daemon restart. Stable durable-derived span IDs make invocation replay safe; additive metrics use `otlp_metric_projections` as a submitted-to-SDK restart checkpoint, not an exactly-once transport guarantee.
- Export is metadata-only, bounded, asynchronous, and fail-independent. Never add prompts, responses, logs, diffs, files, command output, raw errors, arbitrary URLs, check names, session identities, branch or commit identities, secrets, caller-selected attributes, or content-capture controls. Token components export only when exactly reported; resumed cumulative deltas fail closed on missing or invalid predecessors. `internal/oteltrace/registry.go` owns approved names and dimensions; `docs/src/content/docs/reference/environment.md` owns configuration, source semantics, health, bounds, deduplication caveats, and privacy.
- Regressions: `internal/oteltrace/invocation_test.go`, `internal/oteltrace/runtime_test.go`, `internal/daemon/event_subscription_test.go`, e2e `TestNativeOTLPTraceJourney`.
...
- Detailed performance evidence is local in `agent_invocations` plus `runs.parked_ms`; never store prompts, outputs, diffs, or raw command arguments there (shape-guard test `TestAgentInvocations_PrivacySafeShape`). Never send run IDs, paths, session identities, or per-invocation records to Umami - its only remote perf data is three bounded counts on the terminal `run finished` event. Separately configured native OTLP may export only the approved bounded invocation projection above. The local/remote split is documented in `docs/src/content/docs/reference/environment.md`; read locally with `no-mistakes stats`.
