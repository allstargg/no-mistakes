...
func (s *CIStep) ReconcileApprovalGate(sctx *pipeline.StepContext) (bool, error) {
	...
	case scm.PRStateMerged:
		if err := sctx.DB.UpdateRunPRStateWithEvent(sctx.Ctx, sctx.Run.ID, db.PRStateMerged); err != nil {
			return false, err
		}
		if sctx.Log != nil {
	...
	case scm.PRStateClosed:
		if err := sctx.DB.UpdateRunPRStateWithEvent(sctx.Ctx, sctx.Run.ID, db.PRStateClosed); err != nil {
			return false, err
		}
		if sctx.Log != nil {
	...
	case scm.PRStateOpen:
		if err := sctx.DB.UpdateRunPRStateWithEvent(sctx.Ctx, sctx.Run.ID, db.PRStateOpen); err != nil {
			return false, err
		}
		return false, nil
	default:
...
func (s *CIStep) Execute(sctx *pipeline.StepContext) (*pipeline.StepOutcome, error) {
	...
	for {
		...
		if err != nil {
			...
			if err := sctx.DB.UpdateRunPRStateWithEvent(ctx, sctx.Run.ID, db.PRStateMerged); err != nil {
				return nil, err
			}
			sctx.Log("PR has been merged!")
			return &pipeline.StepOutcome{}, nil
		} else if state == scm.PRStateClosed {
			if err := sctx.DB.UpdateRunPRStateWithEvent(ctx, sctx.Run.ID, db.PRStateClosed); err != nil {
				return nil, err
			}
			sctx.Log("PR has been closed")
			return &pipeline.StepOutcome{}, nil
		} else if state == scm.PRStateOpen {
			if err := sctx.DB.UpdateRunPRStateWithEvent(ctx, sctx.Run.ID, db.PRStateOpen); err != nil {
				return nil, err
			}
		}
		...
		if err != nil {
			if dbErr := sctx.DB.UpdateRunCIWithEvents(ctx, sctx.Run.ID, db.CIObservation{State: db.CIStateRunning, Outcome: db.CIOutcomeUnknown}); dbErr != nil {
				sctx.Log(fmt.Sprintf("warning: could not persist CI lifecycle: %v", dbErr))
			}
			lastMonitorLog = ""
			...
			observation := db.CIObservation{State: db.CIStateRunning, Outcome: db.CIOutcomeChecks}
			switch {
			case hasIssues:
				observation.State = db.CIStateFailure
				switch {
				case hasFailures && mergeConflict:
					observation.Outcome = db.CIOutcomeChecksAndMergeConflict
				case mergeConflict:
					observation.Outcome = db.CIOutcomeMergeConflict
				default:
					observation.Outcome = db.CIOutcomeChecks
				}
			case pending || (len(checks) == 0 && elapsed < s.gracePeriod()):
				observation.State = db.CIStateRunning
				observation.Outcome = db.CIOutcomeChecks
			default:
				observation.State = db.CIStateGreen
				observation.Outcome = db.CIOutcomePassed
				if len(checks) == 0 {
					observation.Outcome = db.CIOutcomeNoChecks
				}
				observation.ReviewReady = prStateKnown && mergeabilityKnown
			}
			if dbErr := sctx.DB.UpdateRunCIWithEvents(ctx, sctx.Run.ID, observation); dbErr != nil {
				sctx.Log(fmt.Sprintf("warning: could not persist CI lifecycle: %v", dbErr))
			}
