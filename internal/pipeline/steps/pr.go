...
func (s *PRStep) Execute(sctx *pipeline.StepContext) (*pipeline.StepOutcome, error) {
	...
	if existing != nil {
		...
		if updated != nil && updated.URL != "" {
			if err := sctx.DB.RecordRunPROpenWithEvent(ctx, sctx.Run.ID, updated.URL, false); err != nil {
	...
	if err := sctx.DB.RecordRunPROpenWithEvent(ctx, sctx.Run.ID, created.URL, true); err != nil {
