...
func registerHandlers(srv *ipc.Server, mgr *RunManager, d *db.DB, eventNotifier *eventNotifier, nativeOTLP *oteltrace.Runtime, shutdown func()) {
	...
	srv.Handle(ipc.MethodRerun, func(ctx context.Context, params json.RawMessage) (interface{}, error) {
		...
		runID, err := mgr.HandleRerun(ctx, p.RepoID, p.Branch, p.SkipSteps, p.Intent, p.TraceContext)
		if err != nil {
			return nil, err
		}
...
func runToInfo(d *db.DB, r *db.Run, steps []*db.StepResult) *ipc.RunInfo {
	...
	if r.Traceparent != nil {
		info.TraceContext = &tracecontext.Context{Traceparent: *r.Traceparent}
		if r.Tracestate != nil {
			info.TraceContext.Tracestate = *r.Tracestate
		}
	}
	if len(steps) > 0 {
